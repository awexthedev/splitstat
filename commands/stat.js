module.exports = {
    ...
    async execute(client, discord, message, args, fetch) {
        if (!args.length) {
            return message.channel.send(`Hey ${message.author}, you didn't provide your Steam name!`)
        } else {
            const username = args.slice(0).join(' ');
            var url = `https://public-api.tracker.gg/v2/splitgate/standard/profile/steam/${username}`
            ...
            if(!data.data || data.data.segments === undefined) {
                ...
                .setColor(`#2c1178`)
                .setDescription(`An incorrect ID was provided or there is an internal API error.\nTo fix this, make sure you used your Steam64 DEC ID to search. (You can find that [here](https://www.steamidfinder.com/). See **spl!idhelp** for more information.)`)
                .setFooter(`SplitStat`)
                .setTimestamp();
                ...
                collector.on('collect', async (msg) => {
                    if(msg.content.toLowerCase().startsWith(`portal`)) {
                        ...
                        message.channel.send(portalEmbed)
                        ...
                        return message.channel.send(streakEmbed)
                        ...
                        return message.channel.send(accuracyEmbed)
                    } else {
